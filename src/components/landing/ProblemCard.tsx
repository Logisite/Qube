...
import { motion, MotionValue, useTransform, useMotionValueEvent, animate } from "motion/react"
...
export function ProblemCard({ y, scale, borderRadius, opacity, blur, progress }: ProblemCardProps) {
  ...
  useMotionValueEvent(progress, "change", (v) => {
    ...
    if (v >= 0.17 && !leftPlayed && refLeft.current) {
      setLeftPlayed(true)
      animate(refLeft.current, { opacity: 1, x: 0 }, { type: "spring", stiffness: 120, damping: 20 })
    }
    if (v >= 0.24 && !rightPlayed && refRight.current) {
      setRightPlayed(true)
      animate(refRight.current, { opacity: 1, x: 0 }, { type: "spring", stiffness: 120, damping: 20 })
    }
  })
