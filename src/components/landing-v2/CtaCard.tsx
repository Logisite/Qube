...
export function CtaCard({ sectionId }: CtaCardProps) {
  const { y, scale, opacity } = useScrollSection(sectionId)
      ...
      <div className="max-w-6xl mx-auto w-full grid grid-cols-1 lg:grid-cols-12 gap-12 items-center py-4">
        <div className="lg:col-span-5 flex flex-col justify-center">
          ...
          <p className="text-neutral-400 text-sm md:text-base leading-relaxed">
            {pairs.length} official pairs. Zero deployment. {chainConfig.isTestnet ? "Free on" : "Live on"} {chainConfig.shortName}. Just connect and go.
          </p>

          <div className="mt-8 flex items-center gap-6 border-t border-neutral-800 pt-6">
            <div>
              <span className="block text-2xl font-black text-white">{pairs.length}</span>
          ...
          <div className="absolute left-6 bottom-6 right-6 z-10 flex flex-col md:flex-row gap-4 items-stretch">
            <div className="flex-1 bg-neutral-900/90 border border-neutral-800 rounded-xl p-4 backdrop-blur-md shadow-2xl">
              <div className="flex justify-between items-center mb-1.5">
                ...
                <span className="text-[9px] text-neutral-500 font-mono">{chainConfig.shortName}</span>
              </div>
              <p className="text-xs font-bold text-white leading-snug">{registryAddress}</p>
            </div>
